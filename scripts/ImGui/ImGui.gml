function imgui() constructor {
	...
	static __Update = function() {
		...
		if (!surface_exists(__Surface)) {
			__Surface = surface_create(max(window_get_width(),10), max(window_get_height(),10));	
		}
		...
		if (buffer_peek(__FontBuffer, 0, buffer_bool)) {
			...
			var font = surface_create(max(buffer_peek(__FontBuffer, 1, buffer_u32), 10), max(buffer_peek(__FontBuffer, 5, buffer_u32), 10));
